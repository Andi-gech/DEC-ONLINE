...
export default function EnrolledCouse() {
  ...
  {error && <div className='absolute top-[1px] right-[15%]'><ErrorPopup error={error.message}/></div>}
   ...
   <>
    <Link to={`/enroll/${course?._id}`} className='w-[300px] shadow-md rounded-md m-2 shadow-gray-200 dark:shadow-gray-800 cursor-pointer hover:scale-[1.05]  transform transition-transform  duration-1000 hover:opacity-90  mx-2  h-[300px]'> <img src={`https://dec-online-otal.vercel.app/images/${course?.image}`} className='w-full h-[50%] rounded-md'/>
